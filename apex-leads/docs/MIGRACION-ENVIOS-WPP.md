...
WHERE slug = 'carta';
```
**Assistify** (no es la queue activa): su template también manda un link en frío (`assistify.lat/download`). Mismo principio aplica, pero es producto self-serve (el link es descarga de app, no un boceto por-lead → el `[BOCETO]` no le sirve; el link de descarga debería vivir en `project_info` para que el agente lo mande al responder). **DECIDIDO (sesión 3): SÍ, implementar** — principio global de Manuel: ningún proyecto manda link en el primer mensaje. Pendiente de hacer en sesión nueva (cambio acoplado, ver §"Estado de implementación (sesión 3)").
...
- **Hook de auto-deploy: PROBADO en vivo.** El Stop hook pusheó solo `da02912` y `bbfc848` a `origin/master` → Vercel deployó. Funciona de punta a punta.
- **Resend (email de baneo): RESUELTO.** Bug raíz: el remitente `alerts@apex-leads.com` (en `sender-alerts.ts` **y** `ig/alerts.ts`) NO está verificado en Resend → los emails fallaban en silencio. Cambiado a `alerts@botlode.com` (dominio verificado de Manuel) en ambos archivos (commit `bbfc848`, `tsc` 0 · `jest` 293/293). Manuel cargó `RESEND_API_KEY` + `ALERT_EMAIL` (=`manunv97@gmail.com`) en Vercel (Production). Como las env vars no entran en builds previos, se forzó redeploy `5d49500` → **READY en `leads.theapexweb.com`** con vars + remitente verificado. Prueba de entrega real: **HECHA** — email de prueba enviado OK vía Resend API (id `b9114c1d-6c64-4835-b7ab-7fbb06dc330d`, from `alerts@botlode.com` → `manunv97@gmail.com`); key válida + dominio `botlode.com` verificado confirmados. La key quedó también en `apex-leads/.env.local` (gitignored).
